...
class IcarusDataPak:
    ...
    def _decompressed_blob(self) -> bytes:
        ...
        while pos < end:
            if raw[pos : pos + 2] != b"\x78\x9c":
                nxt = raw.find(b"\x78\x9c", pos + 1, end)
                if nxt == -1:
                    break
                pos = nxt
            ...
            if not d.eof:
                break
            used = (end - pos) - len(d.unused_data)
            pos += used
...
class MountDetails(QWidget):
    ...
    def __init__(self) -> None:
        ...
        g_form.addRow("Lineage", self.cmb_lineage)
    ...
    def set_mount(self, mount: Optional[Dict[str, Any]]) -> None:
        ...
        if mtype_s and self.cmb_type.findText(mtype_s) < 0:
            self.cmb_type.addItem(mtype_s)
        self.cmb_type.setCurrentText(mtype_s)
    ...
    def _lineage_changed(self, _text: str) -> None:
        ...
        if not data:
            return
        row_name = self.cmb_lineage.currentData()
        if not isinstance(row_name, str) or not row_name.strip():
            row_name = self.cmb_lineage.currentText().strip()
        if not row_name:
            return
        changed = mount_blob_set_fstring(data, "Lineage", "NameProperty", row_name)
        mount_blob_set_fstring(data, "LineageName", "NameProperty", row_name)
        if changed:
            self.changed.emit()
    ...
    def _type_changed(self, text: str) -> None:
        ...
        data = self._blob_data()
        if data is not None:
            actor_class = self._guess_actor_class_name(mtype)
            if actor_class:
                mount_blob_set_fstring(data, "ActorClassName", None, actor_class)

                obj = mount_blob_get_fstring(data, "ObjectFName", "NameProperty") or ""
                ...
                path = (
                    mount_blob_get_fstring(data, "ActorPathName", "StrProperty") or ""
                )
                if "." in path:
    ...
    def _talent_rank_set_or_add(self, row_name: str, v: int) -> None:
        ...
        if not data:
            return
        if mount_blob_add_talent(data, row_name, int(v)):
            self.changed.emit()
            self._refresh_talents()
    ...
    def _add_talent_dialog(self) -> None:
        ...
        # offer known talents for current type first
        mtype = ""
        if self.mount:
            t = self.mount.get("MountType", "")
            mtype = t if isinstance(t, str) else ""
        choices: List[str] = []
        for rn in (self._talent_catalog.get(mtype, []) if mtype else []):
            if rn and rn not in choices:
                choices.append(rn)
        ...
        for rn in choices:
            meta = self._talent_info.get(rn)
            title = meta.display_name if meta and meta.display_name else rn
            cmb.addItem(f"{title} ({rn})", rn)
        try:
        ...
        if not isinstance(rn, str) or not rn.strip():
            ...
            m = re.search(r"\(([^)]+)\)\s*$", raw)
            rn = m.group(1).strip() if m else raw
        rn = rn.strip()
        if not rn:
            return
...
class PetsTab(QWidget):
    ...
    def _copy_mount_icon(self, src_icon: Optional[int], dst_icon: int) -> None:
        if not self.model.root or not src_icon:
            return
        d = os.path.join(self.model.root, "Mounts")
        if not os.path.isdir(d):
            return
        src = os.path.join(d, f"{int(src_icon)}.exr")
    ...
    def _set_row(self, row: int, mount: Dict[str, Any]) -> None:
        ...
        it_name.setFlags(it_name.flags() & ~Qt.ItemIsEditable)
        it_type = QTableWidgetItem(mtype_s)
...
class TestTab(QWidget):
    def __init__(self, model: SaveModel, mark_dirty_cb) -> None:
        ...
        warn = QLabel(
            "Эксперимент: можно создать «кастомного питомца», подменив ActorClassName внутри RecorderBlob.\n"
            "Это НЕ гарантирует работоспособность для любых мобов: многие классы не совместимы с системой маунтов/питомцев."
        )
        warn.setStyleSheet("color:#B5BAC1;")
        warn.setWordWrap(True)
        cp_root.addWidget(warn)

        self.lbl_decompile = QLabel("decompile: (сканирование не запускалось)")
        ...
        tmpl_lay.addWidget(self.cmb_custom_template, 1)
        tmpl_lay.addWidget(self.btn_custom_reload)
        g_form.addRow("Шаблон", tmpl_row)

        self.ed_custom_name = QLineEdit()
        self.ed_custom_name.setPlaceholderText("Имя питомца…")
        g_form.addRow("Имя", self.ed_custom_name)

        self.cmb_custom_type = QComboBox()
        self.cmb_custom_type.setEditable(True)
        g_form.addRow("MountType", self.cmb_custom_type)

        self.sb_custom_level = QSpinBox()
        self.sb_custom_level.setRange(0, 10**9)
        self.sb_custom_level.setValue(1)
        g_form.addRow("Уровень", self.sb_custom_level)

        self.ed_custom_actor = QLineEdit()
        self.ed_custom_actor.setPlaceholderText("Напр.: BP_NPC_Spider_Character_C")
        g_form.addRow("ActorClassName", self.ed_custom_actor)

        mobs_row = QWidget()
        mobs_lay = QHBoxLayout(mobs_row)
        mobs_lay.setContentsMargins(0, 0, 0, 0)
        mobs_lay.setSpacing(6)
        self.cmb_custom_mob = QComboBox()
        self.cmb_custom_mob.setEditable(True)
        self.cmb_custom_mob.setToolTip(
            "Список формируется из decompile/data_pak_tables (BP_*_C)."
        )
        self.cmb_custom_mob.currentTextChanged.connect(self._custom_mob_selected)
        self.btn_pet_spider = QPushButton("Паук")
        self.btn_pet_spider.setToolTip("Быстро создать питомца-паука (ActorClassName).")
        self.btn_pet_spider.clicked.connect(self._add_spider_pet)
        self.btn_pet_lava = QPushButton("Лавовый выродок")
        self.btn_pet_lava.setToolTip(
            "Быстро создать питомца-лавового выродка (LavaBroodling)."
        )
        self.btn_pet_lava.clicked.connect(self._add_lava_broodling_pet)
        mobs_lay.addWidget(self.cmb_custom_mob, 1)
        mobs_lay.addWidget(self.btn_pet_spider)
        mobs_lay.addWidget(self.btn_pet_lava)
        g_form.addRow("Мобы", mobs_row)

        self.ed_custom_ai = QLineEdit()
        self.ed_custom_ai.setPlaceholderText("Опционально: AISetupRowName")
        g_form.addRow("AISetupRowName", self.ed_custom_ai)

        self.cb_custom_reset_talents = QCheckBox(
            "Сбросить таланты/прогресс у нового питомца"
        )
        self.cb_custom_reset_talents.setChecked(True)
        g_form.addRow("", self.cb_custom_reset_talents)

        self.cb_custom_copy_icon = QCheckBox(
            "Скопировать иконку (Mounts/*.exr) от шаблона"
        )
        self.cb_custom_copy_icon.setChecked(True)
        g_form.addRow("", self.cb_custom_copy_icon)
    ...
    def load(self) -> None:
        ...
        # custom pets
        self.cmb_custom_template.setEnabled(enabled and bool(self.model.mounts_path))
        ...
        self.cmb_custom_type.setEnabled(enabled and bool(self.model.mounts_path))
        self.sb_custom_level.setEnabled(enabled and bool(self.model.mounts_path))
        self.ed_custom_actor.setEnabled(enabled and bool(self.model.mounts_path))
        self.cmb_custom_mob.setEnabled(enabled and bool(self.model.mounts_path))
        self.btn_pet_spider.setEnabled(enabled and bool(self.model.mounts_path))
        self.btn_pet_lava.setEnabled(enabled and bool(self.model.mounts_path))
        self.ed_custom_ai.setEnabled(enabled and bool(self.model.mounts_path))
        self.cb_custom_reset_talents.setEnabled(
            enabled and bool(self.model.mounts_path)
        )
        self.cb_custom_copy_icon.setEnabled(enabled and bool(self.model.mounts_path))
        ...
        self._ensure_decompile_scan_started()
    ...
    def _load_custom_templates(self) -> None:
        ...
        try:
            ...
            self.cmb_custom_type.blockSignals(True)
            self.cmb_custom_type.clear()
            self.cmb_custom_type.blockSignals(False)
            ...
            for t in defaults:
                if t and t not in types:
                    types.append(t)
            if self._game_data:
                for t in self._game_data.mount_types:
                    if t and t not in types:
                        types.append(t)
            for m in mounts_list:
                if not isinstance(m, dict):
            ...
            for t in types:
                self.cmb_custom_type.addItem(t)
        ...
        if self.cmb_custom_template.count() > 0:
            self.cmb_custom_template.setCurrentIndex(0)
            self._custom_template_changed()
    ...
    def _custom_template_changed(self) -> None:
        ...
        lvl = tpl.get("MountLevel", 1)

        name_s = name if isinstance(name, str) and name.strip() else "Custom Pet"
        self.ed_custom_name.setText(name_s + "_custom")

        if isinstance(mtype, str) and mtype.strip():
            self.cmb_custom_type.setCurrentText(mtype.strip())
        try:
            self.sb_custom_level.setValue(int(lvl) if isinstance(lvl, int) else 1)
        except Exception:
            self.sb_custom_level.setValue(1)

        actor_class = ""
        ai = ""
        data = self._mount_blob_data(tpl)
        if data:
            actor_class = mount_blob_get_fstring(data, "ActorClassName") or ""
            ai = mount_blob_get_fstring(data, "AISetupRowName") or ""
        self.ed_custom_actor.setText(actor_class)
        self.ed_custom_ai.setText(ai)
    ...
    def _custom_mob_selected(self, text: str) -> None:
        ...
        if not s:
            return
        # keep ActorClassName as the source of truth for _create_custom_pet
        self.ed_custom_actor.setText(s)
    ...
    def _populate_custom_mobs(self) -> None:
        ...
        mobs = [
            c
            for c in classes
            if isinstance(c, str) and c.startswith("BP_NPC_") and c.endswith("_C")
        ]
        mobs = sorted(set(mobs))
        ...
        try:
            ...
            if mobs:
                self.cmb_custom_mob.addItems(mobs)
            if cur:
        ...
        try:
            comp = QCompleter(mobs, self.cmb_custom_mob)
            comp.setCaseSensitivity(Qt.CaseInsensitive)
            comp.setFilterMode(Qt.MatchContains)
    ...
    def _add_spider_pet(self) -> None:
        actor = (
            self._pick_decompile_class(
                preferred=["BP_NPC_Spider_Character_C"],
                contains_any=["npc_spider", "spider_character"],
            )
            or "BP_NPC_Spider_Character_C"
        )
        self.ed_custom_actor.setText(actor)
        if not self.ed_custom_name.text().strip():
            self.ed_custom_name.setText("Spider_custom")
    ...
    def _add_lava_broodling_pet(self) -> None:
        actor = (
            self._pick_decompile_class(
                preferred=[
                    "BP_NPC_LavaBroodling_C",
                    "BP_NPC_LavaBroodling_WorldSpawn_C",
                ],
                contains_any=["lavabroodling"],
            )
            or "BP_NPC_LavaBroodling_C"
        )
        self.ed_custom_actor.setText(actor)
        if not self.ed_custom_name.text().strip():
            self.ed_custom_name.setText("LavaBroodling_custom")
        self._create_custom_pet()
    ...
    def _create_custom_pet(self) -> None:
        ...
        if not isinstance(template, dict):
            QMessageBox.information(self, "Кастомный питомец", "Выбери шаблон.")
            return

        name = self.ed_custom_name.text().strip() or "Custom Pet"
        mtype = self.cmb_custom_type.currentText().strip()
        level = int(self.sb_custom_level.value())
        actor_class = self.ed_custom_actor.text().strip()
        ai_setup = self.ed_custom_ai.text().strip()
        warns: List[str] = []
        ...
        if data is not None:
            ...
            # align XP with requested level (best effort)
            try:
                xp = int(self._xp_curve_mount.value_at(float(level)))
            except Exception:
                xp = 0
            mount_blob_set_int(data, "Experience", int(xp))
            mount_blob_set_int(data, "MountLevel", int(level))

            if self.cb_custom_reset_talents.isChecked():
                try:
                    for t in mount_blob_list_talents(data):
                        mount_blob_set_int_at_offset(data, t.rank_value_offset, 0)
                except Exception:
        ...
        self._apply_mount_identity(new_mount, new_icon, new_suffix)
        if self.cb_custom_copy_icon.isChecked():
            self._copy_mount_icon(src_icon, new_icon)
